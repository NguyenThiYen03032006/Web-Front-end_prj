...
function renderEmployee(){
    assignee.innerHTML = `<option value="">Chọn người phụ trách</option>`; // clear trước
...
function addTask(){
    ...
    if(fixIndex==-1){
        ...
        let newTask={
            ...
            taskName:inputAddTask.value,// ten nguoi phu trach
        ...
        // in put la yyyy-mm-dd doi lai mm-dd
        let [startYear, startMonth, startDay] = startDate.value.split('-');
        let start = `${startMonth}-${startDay}`;
        ...
        detailProject.taskList[fixIndex].asignDate = start;
        detailProject.taskList[fixIndex].dueDate = end;
...
function fixTaskContainer(id){
    ...
    if(fixIndex!= -1){
        ...
        assignee.value=detailProject.taskList[fixIndex].assigneeName;
